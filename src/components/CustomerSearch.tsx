...
export default function CustomerSearch({ onSelect, selectedCustomer, children }: CustomerSearchProps) {
  ...
  const handleSelectCustomer = (customer: Client) => {
    ...
    toast({
      ...
      description: t('customerSelectedDescription', { name: customer.fullName }),
    })
  }
  ...
  const handleCreateCustomer = async () => {
    if (!newCustomer.fullName || !newCustomer.phone) {
      toast({
        ...
        description: t('fillNameAndPhone'),
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: t('newCustomerCreated'),
      })
      ...
      toast({
        title: t('error'),
        description: t('failedToCreateCustomer'),
        variant: 'destructive',
      })
    } finally {
      setIsCreating(false)
      ...
      <DialogTrigger asChild>
        {children || (
          <Button variant="outline" className="w-full justify-start">
            ...
            {selectedCustomer ? selectedCustomer.fullName : t('selectCustomer')}
          </Button>
        )}
      </DialogTrigger>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <span>{showCreateForm ? t('createNewCustomer') : t('selectCustomer')}</span>
            {!showCreateForm && (
              <Button
                variant="outline"
                size="sm"
