...
export function TechnicianEarningsReport({ technicianId }: TechnicianEarningsReportProps) {
  ...
  const totalPages = Math.max(1, Math.ceil(filteredEarnings.length / itemsPerPage))
  ...
  const getTypeBadge = (type: TechnicianEarning['type']) => {
    if (type === 'COMMISSION') {
      return <Badge className="bg-green-100 text-green-800 hover:bg-green-100">Commission</Badge>
    }

    return <Badge className="bg-red-100 text-red-800 hover:bg-red-100">Penalty</Badge>
  }
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">
              {formatCurrency(Math.abs(totalPenalties))}
            </div>
            <p className="text-xs text-muted-foreground">
              ...
              {formatDate(earning.createdAt)}
                      ...
                      <TableCell>
                        <span className={`font-bold ${earning.amount >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {formatCurrency(earning.amount, { signed: true })}
                        </span>
                      </TableCell>
                      {!technicianId && (
                        <TableCell>
